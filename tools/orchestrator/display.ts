...
import type { DependencyNode, OrchestratorState } from '@tools/orchestrator/types.ts';
...
export function printParallelPlan(
	...
	deps.log.step(`EXECUTION PLAN (parallel: ${parallelN})`);
	for (let t = 0; t < tiers.length; t++) {
		const tier = tiers[t];
		const concurrent = Math.min(tier.length, parallelN);
		const afterLabel = t === 0 ? '' : `, after tier ${t - 1}`;
		deps.consolelog(`  Tier ${t} (${concurrent} concurrent${afterLabel}):`);
		for (const num of tier) {
			...
			const depStr =
				node.dependsOn.length > 0
					...
					: ' (no deps)';
			deps.consolelog(`    #${num} ${node.issue.title}${depStr}`);
		}
	}
...
export function printExecutionPlan(
	...
	for (let i = 0; i < order.length; i++) {
		...
		deps.consolelog(`  ${(i + 1).toString().padStart(2)}. #${num} ${node.issue.title}${depStr}`);
...
export function printStatus(state: OrchestratorState, deps: DisplayDeps = defaultDisplayDeps): void {
	deps.log.step('ORCHESTRATOR STATUS');
	...
	deps.consolelog(`  Started: ${state.startedAt}`);
	deps.consolelog(`  Updated: ${state.updatedAt}`);
	const blockedLabel = blocked > 0 ? `, ${blocked} blocked` : '';
	deps.consolelog(`  Progress: ${completed} completed, ${failed} failed${blockedLabel}, ${pending} pending\n`);
	...
	for (const entry of entries) {
		...
		deps.consolelog(`  ${icon} #${entry.number}${titleStr} [${entry.status}]${extra}${errMsg}`);
	}
}
